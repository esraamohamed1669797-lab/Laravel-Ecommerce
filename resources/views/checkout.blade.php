            ...
            <div class="sticky-content">
              <div class="checkout__totals">
                  ...
                  <tbody>
                    ...
                    <tr>
                     ...
                     {{$item->name}}x {{$item->qty}}
                      ...
                      <td align="right">
                       EGP {{ number_format((float) str_replace(',', '', $item->subTotal()), 0) }}
                      </td>
                    </tr>
                   @endforeach
                  </tbody>
                ...
                @if (Session::has('discounts'))
                 ...
                 <tbody>
                  <tr>
                    <th>Subtotal</th>
                    <td class="text-right" >EGP {{ number_format((float) str_replace(',', '',Cart::instance('cart')->subtotal()),0)}}</td>
                  </tr>
                   <tr>
                    <th>Discount{{Session::get('coupon')['code']}}</th>
                    <td class="text-right">EGP {{ number_format((float) str_replace(',', '',Session::get('discounts')['discount']),0)}}</td>
                  </tr>
                   <tr>
                    <th>Subtotal After Discount</th>
                    <td class="text-right">EGP {{ number_format((float) str_replace(',', '',Session::get('discounts')['subtotal']),0)}}</td>
                  </tr>
                  <tr>
                    <th>Shipping</th>
                    <td class="text-right">
                  ...
                  <tr>
                    <th>VAT</th>
                    <td class="text-right">EGP {{ number_format((float) str_replace(',', '',Session::get('discounts')['tax']),0)}}</td>
                  </tr>
                  <tr>
                    <th>Total</th>
                    <td class="text-right">EGP {{ number_format((float) str_replace(',', '',Session::get('discounts')['total']),0)}}</td>
                  </tr>
                </tbody>
                 </table>
                @else
                <table class="checkout-totals">
                  <tbody>
                    ...
                    <tr>
                      <th>VAT</th>
                      <td class="text-right">EGP {{ number_format((float) str_replace(',', '',Cart::instance('cart')->tax()),0)}}</td>
                    </tr>
                    <tr>
                      <th>TOTAL</th>
                      <td class="text-right">EGP {{ number_format((float) str_replace(',', '',Cart::instance('cart')->total()),0)}}</td>
                    </tr>
                  </tbody>
                  @endif
                </table>
              ...
              <div class="checkout__payment-methods">
                ...
                <div class="policy-text">
                  ...
                  website, and for other purposes described in our <a href="{{route('cart.order.confirmation')}}">privacy
                    policy</a>.
                </div>
              </div>
