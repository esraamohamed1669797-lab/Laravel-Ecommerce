                ...
                <div class="col-lg-5">
                    ...
                    <div class="product-single__price">
                        <span class="current-price">
                            @if ($product->sale_price)
                                ...
                                <s>EGP {{ number_format($product->regular_price, 0) }}</s>

                                <span>{{ $product->sale_price }}% OFF</span>

                                <h5> EGP {{ number_format($finalPrice, 0) }}</h5>
                            @else
                                <h5>EGP {{ number_format($product->regular_price, 0) }}</h5>
                            @endif
                        </span>
                    </div>
                    ...
                    @else
                        <form name="addtocart-form" method="post" action="{{ route('cart.add') }}"
                            class="add-to-cart-form" data-product="{{ $product->id }}">
                            @csrf
                    ...
                    <div class="product-single__addtolinks">
                        <form action="{{ route('wishlist.toggle') }}" method="POST" class="wishlist-form"
                            ...
                            <button type="submit"
                                class="menu-link menu-link_us-s add-to-wishlist border-0 bg-transparent p-0">
                                <i
                                    class="fa {{ Cart::instance('wishlist')->content()->where('id', $product->id)->count() ? 'fa-heart text-danger' : 'fa-heart-o' }}"></i>
                                <span class="wishlist-text">
                                    {{ Cart::instance('wishlist')->content()->where('id', $product->id)->count() ? 'Remove From Wishlist' : 'Add To Wishlist' }}
                                </span>
                            </button>
                        </form>
                    </div>
    ...
    @foreach ($rproducts as $rproduct)
        ...
        @php
            if ($rproduct->sale_price) {
                $discountAmount = ($rproduct->regular_price * $rproduct->sale_price) / 100;
                $finalPrice = $rproduct->regular_price - $discountAmount;
            } else {
                $finalPrice = $rproduct->regular_price;
            }
        @endphp
        ...
        <div class="swiper-slide product-card">
            <div class="pc__img-wrapper">
                ...
                @else
                    ...
                    <form method="POST"
                        ...
                        class="add-to-cart-form related-cart-form"
                        data-product="{{ $rproduct->id }}">
            ...
            <div class="pc__info position-relative">
                ...
                <div class="product-card__price d-flex">
                    <span class="money price">
                        ...
                        @if ($rproduct->sale_price)
                            ...
                            <s>EGP {{ number_format($rproduct->regular_price, 0) }}</s>

                            <span>{{ $rproduct->sale_price }}% OFF</span>

                            <h5> EGP {{ number_format($finalPrice, 0) }}</h5>

                        @else

                            <h5>EGP {{ number_format($rproduct->regular_price, 0) }}</h5>
                ...
                <form action="{{ route('wishlist.toggle') }}"
                    method="POST"
                    class="wishlist-form position-absolute top-0 end-0"
                    data-product="{{ $rproduct->id }}">
                    ...
                    <button type="submit"
                        ...
                        <i class="fa {{ Cart::instance('wishlist')->content()->where('id', $rproduct->id)->count() ? 'fa-heart text-danger' : 'fa-heart-o' }}"></i>
